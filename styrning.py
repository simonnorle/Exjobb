...
def Frequency(
   ...
   # import time    #Used for benchmark testing
    ...
    time_hours = 24
    Freq_data = r'/Users/simonnorle/Documents/Uppsala - år 5/Exjobb/'+str(year)+' 10Hz/' + str(date_str) +'.csv' #Finns kanske ett bättre sätt att göra detta
    Frequency = np.array(pd.read_csv(Freq_data,sep=',',usecols=[1]))

    ### This part is for identifying indices where data is missing
    time_data = np.array(pd.read_csv(Freq_data,sep=',',usecols=[0]))
    time_date = []
    missing_delta_t = []
    missing_index = []
    ...
    for i in range(time_hours): # -1 då index börjar från 0. Gäller för int(), men behövs det för len(list)?
        ...
        FCR_N_count = 0 
    return Flex_frac_FCR_D, Flex_frac_FCR_U, Flex_frac_FCR_N, missing_index[0]
